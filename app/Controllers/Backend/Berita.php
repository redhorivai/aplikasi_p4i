...
class Berita extends BaseController
{
        ...
        if ($this->request->isAJAX()) {
            ...
            if ($id == "") {
                ...
                <form class='form-data form-layout-1 forms'>
                    ...
                    <div class='col-lg-8'>
                        <div class='form-group'>
                        <label class='form-control-label tx-bold'>Judul Berita: <span class='tx-danger'>*</span></label>
    ...
    public function insert_data()
    {
        if ($this->request->isAJAX()) {
            ...
            $insert = $this->m_berita->insertData($data);
            if ($insert == true) {    
                $msg = "Sukses";
            } else {
    ...
    public function update_data()
    {
        if ($this->request->isAJAX()) {
            ...
            $update = $this->m_berita->updateData($id, $data);
            if ($update == true) {    
                $msg = "Sukses";
            } else {
                $msg = "Judul: <b class='text-danger'>$title</b> sudah ada, silahkan coba yang lain.";
    ...
    public function del_data()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = ['sukses' => 'Berita telah dihapus.'];
            echo json_encode($msg);
        } else {
            exit('Request Error');
        }
    ...
    public function multi_del()
    {
        if ($this->request->isAJAX()) {
            ...
            for ($i = 0; $i < $jmldata; $i++){
                ...
                $this->m_berita->updateData($id[$i], $data);
            }
            $msg = ['sukses' => '<b style="margin-left:10px;"> '.$jmldata.' data</b> berita telah dihapus.'];
            echo json_encode($msg);
        } else {
            exit('Request Error');
        }
    ...
    public function detail()
    {
        if ($this->request->isAJAX()) {
            $id      = $this->request->getPost('artikel_id');
            $artikel = $this->m_berita->getByID($id);
