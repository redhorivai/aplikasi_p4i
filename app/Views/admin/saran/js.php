...
<script type="text/javascript">
  ...
  function _getSaran() {
    $('#viewTable').DataTable({
      ...
      "ajax": "<?= site_url('Backend/Saran/getData') ?>",
    });
  }
  ...
  function _delData(id, nama) {
    Swal.fire({
      title: 'Hapus Data?',
      html: `<p class="mg-b-10">Anda akan menghapus Data Saran:</p><p><b>${nama}</b></p>`,
      icon: 'warning',
      showCancelButton: true,
      showConfirmButton: true,
      cancelButtonColor: '#d33',
      ...
      if (result.value) {
        $.ajax({
          type: "POST",
          url: "<?= site_url('Backend/Saran/del_data') ?>",
          data: {
            id: id
          },
          dataType: "JSON",
  ...
  //   if (tglAwal && tglAkhir) {
  //     $.ajax({
  //       url: "<?= site_url('Backend/Lapor/view_report') ?>",
  //       type: "POST",
  //       data: {
  //         tglAwal: tglAwal,
  //         tglAkhir: tglAkhir,
