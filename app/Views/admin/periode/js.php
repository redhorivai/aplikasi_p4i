...
<script type="text/javascript">
    ...
    function _getPeriode() {
        $('#viewTable').DataTable({
          ...
          "ajax": "<?= site_url('Backend/Periode/getData') ?>",
        });
    }
    function _btnAdd() {
        $.ajax({
    ...
    function _simpan() {
        ...
        if (name && username && email && phone && gender && level) {
            $.ajax({
                url: "<?= site_url('Backend/periode/insert_data') ?>",
                type: "POST",
                data: {
                    name     : name,
                    username : username,
                ...
                success: function(response) {
                    if (response == "Sukses") {
                        Toast.fire({
                            icon: "success",
                            title: "Data periode berhasil ditambahkan"
                        });
                        $('.form-data')[0].reset();
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response == "Sukses") {
                        Toast.fire({
                            icon: "success",
                            title: "Data periode telah diperbaharui."
                        });
                        $('.form-data')[0].reset();
                        $('#formData').addClass('d-none');
                        $('#viewData').delay(100).fadeIn();
    ...
    function _deactive(id_periode, nm_periode) {
        Swal.fire({
          title: 'Nonaktif Periode?',
          html: `<p class="mg-b-10">Anda akan menonaktifkan :</p><p><b>${nm_periode}</b></p>`,
          icon: 'question',
          showCancelButton: true,
          showConfirmButton: true,
          cancelButtonColor: '#d33',
        ...
        Swal.fire({
          ...
          confirmButtonText: 'Ya, proses',
            ...
            if (result.value) {
              $.ajax({
                type: "POST",
                url: "<?= site_url('Backend/periode/del_data') ?>",
                data: {
                  user_id: user_id
                },
                dataType: "JSON",
